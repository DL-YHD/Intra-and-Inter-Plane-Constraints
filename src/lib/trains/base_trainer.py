...
class BaseTrainer(object):
  ...
  def run_epoch(self, phase, epoch, data_loader,unlabel_loader1=None,unlabel_loader2=None,unlabel_set=None,iter_num=None,data_loder2=None):
    ...
    for iter_id, batch in enumerate(data_loader):  
      ...
      pcoor_weight=self.rampup_coor(epoch)
      ...
      loss =  loss['hm_loss'].mean() * opt.hm_weight + \
              ...
              loss['planes_hp_offset_loss'].mean() * opt.plane_off_weight +\
              loss['planes_hm_hp_loss'].mean()*opt.planes_hm_hp_weight + \
              loss['plane_hp_loss'].mean()*opt.plane_hp_weight +\
              loss['prob_loss'].mean()*self.rampup_prob(epoch)+\
              loss['coor_loss'].mean()*coor_weight+\
              loss['plane_coor_loss'].mean()*coor_weight
              # loss['pprob_loss'].mean()*self.rampup_prob(epoch)+\
              # loss['pcoor_loss'].mean()*pcoor_weight + \
              #loss['plane_vertical_loss'].mean()*opt.plane_vertical_weight
              #loss['plane_horizon_loss'].mean()*opt.plane_horizon_weight
               ...
               # *loss['coor_loss'].mean() have some trouble*
      # loss = loss['hm_loss'].mean() * opt.hm_weight + \
      #        loss['hp_loss'].mean() * opt.hp_weight + \
      #        loss['dim_loss'].mean() * opt.dim_weight + \
      #        loss['rot_loss'].mean() * opt.rot_weight + \
