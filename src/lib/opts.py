...
class opts(object):
  def __init__(self):
    ...
    self.parser.add_argument('--test_data_used', default='default')
    ...
    self.parser.add_argument('--dataset', default='kitti',
                             help='dataset'
                                  '1.kitti'
                                  '2.nuscenes')
    self.parser.add_argument('--split', default='default',
    			                  help='split'
                                  '1.split_1'
                                  '2.split_2')
    ...
    self.parser.add_argument('--lr_step', type=str, default='50,100,120',
    ...
    self.parser.add_argument('--mse_loss', action='store_true',
                             help='use mse loss or focal loss to train '
                                  'keypoint heatmaps.')
    self.parser.add_argument('--n_p_mse_loss', action='store_true',
                             help='use mse loss or focal loss to train '
                                  'keypoint heatmaps.')
    # ctdet
    ...
    self.parser.add_argument('--plane_off_weight', type=float, default=1,
                             help='loss weight for keypoint local offsets.')
    ...
    self.parser.add_argument('--plane_hp_weight', type=float, default=1,
                             help='loss weight for human pose offset.')
    self.parser.add_argument('--hm_hp_weight', type=float, default=1,
                             help='loss weight for human keypoint heatmap.')
    self.parser.add_argument('--planes_hm_hp_weight', type=float, default=1,
                             help='loss weight for human keypoint heatmap.')
    ...
    self.parser.add_argument('--plane_horizon_weight', type=float, default=0.2,
                             help='plane loss weight for horizon.')
    self.parser.add_argument('--plane_vertical_weight', type=float, default=0.2,
                             help='plane loss weight for horizon.')
    ...
    self.parser.add_argument('--planes_n_dense_hp', action='store_true',
                             help='apply weighted pose regression near center '
                                  'or just apply regression on center point.')
    ...
    self.parser.add_argument('--not_n_points_hm_hp', action='store_true',
                             help='not estimate human joint heatmap, '
                                  'directly use the joint offset from center.')
    self.parser.add_argument('--not_reg_hp_offset', action='store_true',
                             help='not regress local offset for '
                                  'human joint heatmaps.')
    self.parser.add_argument('--not_reg_planes_hp_offset', action='store_true',
                             help='not regress local offset for '
                                  'human joint heatmaps.')
  ...
  def parse(self, args=''):
    ...
    opt.n_points_hm_hp = not opt.not_n_points_hm_hp
    opt.reg_planes_hp_offset = (not opt.not_reg_planes_hp_offset) and opt.n_points_hm_hp
  ...
  def update_dataset_info_and_set_heads(self, opt, dataset):
    ...
    print('==============================================')
    print('n points per every planes',dataset.n_num_joints)
    print('==============================================')
    opt.planes_n_kps = dataset.n_num_joints
    ...
    opt.heads = {'hm': opt.num_classes, 'wh': 2, 'hps': 18, 'planes_n_kps': 6*opt.planes_n_kps*2 ,'rot': 8,'dim':3, 'prob':1}
    # opt.heads = {'hm': opt.num_classes, 'wh': 2, 'hps': 18 ,'rot': 8,'dim':3, 'prob':1}
    if opt.reg_offset:
      opt.heads.update({'reg': 2})
    if opt.hm_hp:
      opt.heads.update({'hm_hp': 9})
    if opt.n_points_hm_hp:
      opt.heads.update({'n_points_hm_hp': 6*opt.planes_n_kps})
    if opt.reg_hp_offset:
      opt.heads.update({'hp_offset': 2})
    if opt.reg_planes_hp_offset:
        opt.heads.update({'planes_n_hp_offset': 6*opt.planes_n_kps*2})
       # opt.heads.update({'planes_n_hp_offset': 6*2})
  ...
  def init(self, args=''):
    default_dataset_info = {
        'PlaneCons_Kitti': {
            ...
            'dataset': 'kitti_hp', 'num_joints': 8, 'n_num_joints':25,
    ...
    opt = self.parse(args)
    dataset = Struct(default_dataset_info['PlaneCons_Kitti'])
