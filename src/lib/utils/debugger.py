...
class_avg_dim = {'Car':[1.52986348, 1.61876715, 3.89206519],
                 'Pedestrian':[1.76773901, 0.62783416, 0.81796103],
                 'Cyclist':[1.72332425, 0.56961853, 1.77081744]
                } 
...
class Debugger(object):
  def __init__(self, ipynb=False, theme='black', 
    ...
    if dataset == 'coco_hp':
      ...
      self.colors_hp = [(255, 0, 255), (255, 0, 0), (0, 0, 255), 
        ...
        self.ec = [(255, 0, 0), (0, 0, 255), 
                   (255, 0, 0), (0, 0, 255),
                   (255, 0, 0), (0, 0, 255), 
                   (255, 0, 0), (0, 0, 255), 
                   (128, 128, 128)]
               
        self.colors_hp = [(0, 0, 255),   (255, 0, 0), (0, 0, 255),(255, 0, 0), 
                          (0, 0, 255),   (255, 0, 0), (0, 0, 255),(255, 0, 0),
                          (125, 125, 125)]
        '''
        self.colors_hp = [(255, 0, 255), (255, 0, 255), (255, 0, 255), (255, 0, 255),
  ...
  def add_bev(self, box, kp_coor, img_id, color=(255,0,0), is_faster = True):
      #position=box[36:39]
      #dim=box[32:35]
      position=box[-5:-2]
      dim=box[-9:-6]
      ...
      ori=box[35]
      if is_faster:
          score_3d = box[4] * (1 / (1 + math.exp(-box[-2])))
      else:
          score_3d = (box[4] + (1 / (1 + math.exp(-box[-2]))) + (sum(results[5+kp_coor:5+kp_coor+keypoints])/keypoints)) / 3
  ...
  def add_kitti_hp(self, points, img_id='default', color_flag='pre'):
    points = np.array(points, dtype=np.int32).reshape(self.num_joints, 2)
    if color_flag == 'pre':
      for j in range(self.num_joints):
        cv2.circle(self.imgs[img_id],
      ...
      for j in range(self.num_joints):
        cv2.circle(self.imgs[img_id],
                  (points[j, 0], points[j, 1]), 3, (0, 255, 0), -1)
    # for j, e in enumerate(self.edges):
    #   if points[e].min() > 0:
    #     cv2.line(self.imgs[img_id], (points[e[0], 0], points[e[0], 1]),
  ...
  '''
  def save_kitti_format(self, results, kp_coor, img_path, opt, img_id='default',is_faster=False):
    # points=points_dim[:16]
    # dim = points_dim[16:19]
    # points = np.array(points, dtype=np.int32).reshape(self.num_joints, 2)
    result_dir=opt.results_dir
    ...
    keypoints = kp_coor//2
    if is_faster:
        score=results[4]*(1/(1+math.exp(-results[-2])))
    else:
        score=(results[4]+(1/(1+math.exp(-results[-2])))+(sum(results[5+kp_coor:5+kp_coor+keypoints])/keypoints))/3   
    pos=results[5+kp_coor+keypoints+3+1:5+kp_coor+keypoints+3+1+3]
    ori=results[5+kp_coor+keypoints+3]
    cat=results[5+kp_coor+keypoints+3+1+3+1]
    det_cats = ['Car', 'Pedestrian', 'Cyclist']
    dim=results[5+kp_coor+keypoints:5+kp_coor+keypoints+3]
    if dim[0] < 0 or dim[1]<0 or dim[2]<0:
        print(det_cats[int(cat)])
        print(file_number,dim)
        if dim[0] < 0:
            dim[0] = class_avg_dim[det_cats[int(cat)]][0]
        if dim[1] < 0:
            dim[1] = class_avg_dim[det_cats[int(cat)]][1]
        if dim[2] < 0:
            dim[2] = class_avg_dim[det_cats[int(cat)]][2]
    self.write_detection_results(det_cats[int(cat)],result_dir,file_number,box,dim,pos,ori,score)
  ...
  def add_3d_detection(self, results, kp_coor, calib, img_id='default',  show_txt=False):
      ...
      pos=results[5+kp_coor+keypoints+3+1:5+kp_coor+keypoints+3+1+3]
      ori=results[5+kp_coor+keypoints+3]
      cat=int(results[5+kp_coor+keypoints+3+1+3+1])
