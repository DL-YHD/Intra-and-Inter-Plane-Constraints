...
def car_pose_post_process(dets, kp_num, planes, c, s, h, w):
  # dets: batch x max_dets x 40 (1, 100, 41)
  # return list of 39 in image coord
  ret = []
  kp_coor = planes*kp_num * 2
  kp_num  = planes*kp_num
  
  for i in range(dets.shape[0]): # i=0
    bbox = transform_preds(dets[i, :, :4].reshape(-1, 2), c[i], s[i], (w, h)) # bbox = (200,2)        
    pts = dets[i, :, 5:5+kp_coor] # 18 points
          #transform_preds(dets[i, :, 5:23].reshape(-1, 2), c[i], s[i], (w, h))
    dim = dets[i, :, 5+kp_coor:8+kp_coor] # 3 dim
    #alpha =get_alpha(dets[i, :, 35:43])[:, np.newaxis].astype(np.float32)
    
    pts_score=dets[i, :, 8+kp_coor:8+kp_coor+kp_num] # 9 pts
    rot_y=dets[i,:,8+kp_coor+kp_num:9+kp_coor+kp_num] # 1 dim
    position=dets[i,:,9+kp_coor+kp_num:12+kp_coor+kp_num] # 3 location
    prob=dets[i, :, 12+kp_coor+kp_num:13+kp_coor+kp_num] # 1 prob
    cat = dets[i, :, 13+kp_coor+kp_num:14+kp_coor+kp_num] # 1 class
    top_preds = np.concatenate(
                              [bbox.reshape(-1, 4), dets[i, :, 4:5],
